...
Musimy więc znaleść sposób na zmniejszanie współczynnika eksploracji w trakcie uczenia. Istnieje wiele podejść, których użyteczność zależy od konkretnego przypadku. Dla przykładu najprostrzym sposobem jest liniowe zmniejszanie współczynnika.
...
Głębokie nauczanie przez wzmacnianie (and. Deep Reinforcement Learning - DRL) jest w wielu aspektach identyczne do nauczania które do tej pory omówiliśmy. Zasadniczą różnicą jest polityka. Dalej ma ona tę samą ideę - w trakcie uczenia jest doskonalona i na podstawie doświaczeń dla danego stanu zwraca akcję. Jednak zamiast sprawdzania wartości w Q-Tabeli, stan środowiska zostaje przepuszczony przez sieć neuronową, zwaną Deep Q-Network, która na wyjściu zwraca najlepszy według siebie stan.

\subsection{Dlaczego sieć neuronowa?}

Mimo że Q-Tabela sprawdza się bardzo dobrze, ma ona kilka sporych ograniczenie. Jednym z największych jest fakt, że przetrzymuje ona jedynie dyskretne stany. Nie jest to problemem jeżeli nasze środowisko jest dyskretne (np. gra w warcaby), ale problem się pojawia dla środowisk ze stanami ciągłymi. W prawdzimym życiu najczęściej spotkamy się ze środowiskami które zawierają elementy przyjmujące wartości z pewnego zakresu (np. dla robota sprzątającego jego pozycja, poziom baterii itd.).
...
\subsubsection{Przewidywanie dla podobnych stanów}

Kolejną dużą zaletą nauczania głębokiego jest fakt, że sieć neuronowa potrafi rozpoznać kiedy stany są do siebie podobne. Dla Q-Tabeli, jeżeli stany różnią się w jakikolwiek sposób, nawet najmniejszy i nieistotny (np. robot sprzątający który ma 90\% baterii a 89\%), to są to dla niej kompletnie różne stany dla których musi się osobno uczyć jak się zachowywać.
...
\subsubsection{Środowiska z dużą liczbą stanów}
...
Sieć neuronowa nie ma takiego problemu. Nie musi przechowywać wszystkich możliwych stanów, lecz tylko wagi wyliczone podczas uczenia. Jej rozmiar zależy jednynie od jej budowy, nie od liczby stanów i akcji.

\subsection{Wady nauczania głębokiego}

Nauczanie głębokie nie jest jednak objektywnie "lepszą wersją"~nauczania używającego Q-Tabeli. Zostało ono stworzone, aby można było zastosować nauczania przez wzmacnianie do innych typów środowisk, lecz pomimo wielu zalet, ma ono również swoje wady. 
...
Naturanlym problemem pracy z sieciami neuronowymi jest to że są to "czerne skrzynki"~- dostają dane na wejściu i zwracają na ich podstawie wynik, ale najczęściej nie wiemy jak to robią. Możemy zobaczyć informację o wagach, neuronach itd, ale trudno zinterpretować który element odpowiada za co. Z tego powodu, w przypadku problemów z głębokim nauczaniem często trudo jest zdiagnozować w czym leży problem.

\subsubsection{Stabilność}

Jednym z największych problemów nauczania głębokiego jest ogólna niestabilność uczenia w porównaniu do metody Q-Tabeli. Po pierwsze sieć neuronowa działa na aproksymacjach, a nie na dokładnych wartościach. Dodatkowo jedna aktualizacja parametrów wpływa na wyniki wielu możliwych stanów, nawet tych których agent jeszcze nie widział. Ponadto, w przeciwieństwie do Q-Tabeli, nie ma dowodu na zbiezność nauczania DQN, co oznacza że model może oscylować lub całkowicie rozbiegać w trakcie nauczania.

\subsection{Double DQN}

Popularnym sposobem na zwiększenie stabilności nauczania głebokiego jest użycie dwóch sieci neuronowych - jednej bieżącej (ang. online network) i jednej docelowej (ang. target network). Pozwala to rozdzielić ocenę akcji od wyboru akcji. W klasycznym nauczaniu głębokim agent używa tej samej sieci do oceny akcji jak i do jej wyboru. Potrafi to prowadzić do oscylacji polityki, przeszacowywania wartości Q przez funkcję "max"~oraz generalnej niestabilności. Można porównać to do psa który goni swój własny ogon.
...
Do porównania tych metod nauczania wykorzystamy specjalnie zaimplementowaną grę "Taxi", opartą na popularnym problemie nauczania maszynowego.

\subsection{Cel gry}

Rozgrywka odbywa się na prostokątnej planszy. Na planszy wyznaczone są specjalne pola gdzie może pojawić się pasażer oraz jego cel. Celem taksówki jest jak najszybsze przemieszczenie się na pozycję pasażera, zabranie go, przemieszczenie się na pozycję celu i odstawienie pasażera. Jeżeli to się uda to pasażer i cel zmieniają pozycję na inne losowe pola specjalne. Na planszy występują również ściany, które taksówka musi omijać.

\subsection{Przykładowa plansza}

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.5]{Images/plansza_przyklad.png}
    \caption{Przykładowy stan gry taxi. Elementy gry opisane poniżej}
    \label{fig:explo}
\end{figure}

\subsection{Elementy gry}

\includegraphics[scale=0.5]{Images/taxi.png} - Taksówka \newline \newline
\includegraphics[scale=0.5]{Images/passanger2.png} - Pasażer \newline \newline
\includegraphics[scale=0.5]{Images/destination.png} - Cel \newline \newline
\includegraphics[scale=0.5]{Images/wall.png} - Ściana \newline \newline
\includegraphics[scale=0.5]{Images/spawnpoint.png} - Możliwe punkty pojawienia się pasażera i celu \newline \newline
